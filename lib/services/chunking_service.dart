...
/// Split [text] into TTS-friendly chunks — one per paragraph.
///
/// A chunk is four things at once: a synthesis call, a playlist item, the
/// highlighted region, and a tap-to-seek target. A paragraph is the natural
/// size for all four, so chunks are **not** packed together up to [maxLen].
///
/// Packing was the old behaviour and it cost a pause: consecutive paragraphs
/// were joined into one chunk, and `sanitizeForTts` flattens the blank line
/// between them to a space, so the paragraph break became inaudible. With one
/// paragraph per chunk the system engine speaks each as its own utterance —
/// a real pause — and Piper gets a separate playlist item. It also cuts
/// time-to-first-audio, since only the first paragraph must synthesise before
/// playback starts, rather than up to [maxLen] characters of it.
///
/// [maxLen] is now only a ceiling: a paragraph longer than it is divided by
/// [splitSentences].
///
/// The paragraph pass depends on `cleanText` having preserved blank lines. It
/// once did not, so this branch was dead and every document was cut by
/// [splitSentences] alone — see the note in `text_cleaner.dart`.
