...
///
/// The leading `\s*` matters: "p. 45 [12]." must become "p. 45." and not
/// "p. 45 .", which leaves an orphaned full stop that a later cleanup rule
/// then deletes, costing the sentence its ending.
final _footnoteRegex = RegExp(r'\s*\[\s*\d+(?:\s*[-,–]\s*\d+)*\s*\]');
...
const _headingMaxLength = 60;

/// Give a heading the intonation of a finished statement.
///
/// Chunks here are sentences, so a heading is a very short chunk with no
/// terminal punctuation — "ENDS", "Chapter Four", "Who I Answer To". Without a
/// full stop a synthesiser reads it with trailing, unfinished intonation and
/// runs it into whatever follows.
///
/// The 60-character bound is deliberately tighter than a heading needs: the
/// chunker hard-splits an over-long clause mid-word as a last resort, and those
/// fragments must not be mistaken for headings and given a spurious pause.
///
